let handler = async (m, { conn, args }) => {
  if(!args[0]) return conn.reply(m.chat, `Escribe el numero para tagearlo aunque no este en el grupo`, m) 
  ...
  if(args[0][0] == 0 ||  args[2][0] == 0) return conn.reply(m.chat, 'El número debe empezezar por el código de país y no 0, ejemplo ', m) 
  if(isNaN(args[0])) return conn.reply(m.chat, 'Debe ser un número, no '+args[0], m) 
  if(isNaN(args[2])) return conn.reply(m.chat, 'Debe ser un número, no '+args[2], m) 
  let user1 = `${args[0]}@s.whatsapp.net`
