...
handler.before = async function (m) {
  ...
  if (room) {
    let win = ''
    let tie = false
    if (m.sender == room.p2 && /^(acc(ept)?|terima|gas|aceptare?|nao|gamau|rechazar|ga(k.)?bisa)/i.test(m.text) && m.isGroup && room.status == 'wait') {
      if (/^(tolak|gamau|rechazar|ga(k.)?bisa)/i.test(m.text)) {
        this.reply(m.chat, `@${room.p2.split`@`[0]} rechazado el pvp, el juego se cancela`, m)
...
this.sendButton(m.chat, `*[❗] ${user} 𝙽𝙾 𝙴𝙻𝙴𝙶𝙸𝚂𝚃𝙴 𝙽𝙸𝙽𝙶𝚄𝙽𝙰 𝙾𝙿𝙲𝙸𝙾𝙽, 𝙵𝙸𝙽 𝙳𝙴𝙻 𝙿𝚅𝙿*`.trim(), wm, null, [['𝙼𝙴𝙽𝚄 𝙿𝚁𝙸𝙽𝙲𝙸𝙿𝙰𝙻', '.menu']], m)    
...
return !0
    ...
    if (jwb && reg.test(m.text) && !room.pilih && !m.isGroup) {
      ...
      room.text = m.text
      m.reply(`Tu elejiste ${m.text} ${!room.pilih2 ? `\n\nEsperando a que elija el oponente` : ''}`)
